...
where merchandise cost alone consumes 94% of what an order recognises. On top
of that, the company has been applying one discount policy to two businesses
with completely different tolerance for it.
...
The CFO's framing, what it costs us to generate each rupee, is the right one,
and answered: in Electronics under promotion, more than a rupee.
...
1. **Category, subcategory and price band are the same finding.** 74.9% of
   ...
   independent explanations. Within Electronics, the loss concentrates further:
   Smartphones are 31.2% of *company* revenue at −9.1% margin, losing ₹3.70M,
   against company-wide contribution of ₹11.6M.
...
loss-making category, against 41.9% for Family. The segment axis is silently
carrying a product-mix effect.
...
comparison that does not control for category is a category-mix comparison in
disguise.** The same trap is waiting in acquisition channel, region and
membership tier.
...
- **(d) What we would ask for first.** Promotion incrementality, from a holdout
  or a historical A/B, to distinguish demand the promotion *created* from demand
  it *discounted*.
...
**A fourth we would take if offered:** returns and refunds as financial
reversals. `Return_Flag` records that a return happened but no value flows back
through `Net_Revenue` or `Profit`, so every margin in this analysis is
optimistic by an unknown amount that is largest wherever return rates are
highest.
