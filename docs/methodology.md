...
`data/raw/data-AuroraCart.csv` — the case's `data.csv`, kept under the name it
...
new levels — confirmed by checking that the variants' distributions match their
canonical twin rather than forming a distinct population.
...
| `Days_Since_Last_Purchase` | First observed order — there is no previous purchase | **Leave NaN.** Zero would mean "bought yesterday" |
