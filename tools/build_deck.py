...
def _para(frame, text, *, size, bold=False, color=INK_PRIMARY, space_after=6,
...
def _add_content_slide(prs, slide: Slide):
    ...
    if slide.bullets:
        ...
        for i, bullet in enumerate(slide.bullets):
            _para(bullet_frame, bullet, size=14 if slide.figure else 18,
                  color=INK_SECONDARY, first=(i == 0), space_after=7)
...
def _add_closing_slide(prs, slide: Slide):
    ...
    for i, block in enumerate(slide.bullets):
        ...
        _para(text, f"PRIORITY {i + 1}", size=10, bold=True, color=INK_MUTED, first=True, space_after=6)
