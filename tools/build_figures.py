...
def fig_growth_vs_margin(df) -> go.Figure:
    ...
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.09, row_heights=[0.56, 0.44]
    )
    fig.add_bar(
        x=years.index,
        y=years["Revenue"] / 1e6,
    ...
    fig.add_scatter(
        ...
        text=[f"{v:.1f}%" for v in years["Margin_Pct"]],
    ...
    fig.add_annotation(
        text="▼ margin, below the 9% watch band",
        xref="paper",
        yref="paper",
        x=1,
        y=0.30,
    ...
    fig.update_yaxes(
        title_text="Net revenue (₹M)", range=[0, years["Revenue"].max() / 1e6 * 1.22], row=1, col=1
    )
    fig.update_yaxes(
        title_text="Profit margin (%)", range=[0, years["Margin_Pct"].max() * 1.35], row=2, col=1
    )
    _note(
        fig,
        "Net revenue and margin exclude cancelled orders (zero recognised revenue). "
...
def fig_accelerate_split(df) -> go.Figure:
    ...
    for name, column, color in [
        ("Net revenue per month", "Revenue_Per_Month", C_REVENUE),
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="₹ millions per month", range=[0, (era["Revenue_Per_Month"] / 1e6).max() * 1.3]
        ),
        bargap=0.45,
        bargroupgap=0.12,
    ...
    _note(
        fig,
        f"18 months either side of July 2024. Margin {era['Margin_Pct'].iloc[0]:.1f}% → "
...
def fig_margin_decomposition(df) -> go.Figure:
    ...
    fig = go.Figure(
        go.Waterfall(
            ...
            x=[
                f"{d.base_year} margin",
                "Category mix shift",
                "Margin within categories",
                "Interaction",
                f"{d.final_year} margin",
            ],
            y=[d.base_margin, d.mix_effect, d.rate_effect, d.interaction, d.final_margin],
            text=[
                f"{d.base_margin:.1f}%",
                f"{d.mix_effect:+.1f} pts",
                f"{d.rate_effect:+.1f} pts",
                f"{d.interaction:+.1f} pts",
                f"{d.final_margin:.1f}%",
            ],
            textposition="outside",
            textfont=dict(color=INK["secondary"]),
    ...
    _note(
        fig,
        "Mix effect re-weights base-year category margins with the final-year revenue mix; "
...
def fig_category_margin(df) -> go.Figure:
    ...
    fig = go.Figure(
        go.Bar(
            x=categories["Margin_Pct"],
            y=list(categories.index),
            orientation="h",
            marker=dict(
                color=categories["Margin_Pct"],
                colorscale=DIVERGING_BLUE_RED,
                cmid=0,
                cornerradius=4,
            ),
            text=[
                f"{m:+.1f}%  ({s:.0f}% of revenue)"
                for m, s in zip(
                    categories["Margin_Pct"], categories["Revenue_Share_Pct"], strict=True
                )
            ],
            textposition="outside",
            textfont=dict(color=INK["secondary"]),
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title="Profit margin (%)",
            range=[
                categories["Margin_Pct"].min() - span * 0.18,
    ...
    _note(
        fig,
        "Valid-revenue orders only. Average discount is within 0.3 points across all five "
...
def fig_cost_structure(df) -> go.Figure:
    ...
    for column, label, color in components:
        fig.add_bar(
            ...
            textfont=dict(color=SURFACE, size=11),
    ...
    fig.update_layout(
        ...
        bargap=0.42,
        title=(
            f"Electronics merchandise alone costs {electronics_product:.0f}% "
            "of what the order earns"
        ),
    ...
    _note(
        fig,
        "Costs as recorded in the dataset. Anything the stack pushes past 100% is a loss on "
...
def fig_discount_tolerance(df) -> go.Figure:
    ...
    for name, color in [("Rest of business", CATEGORICAL[0]), ("Electronics", CATEGORICAL[1])]:
        series = (
            side[side["Side"] == name].set_index("Discount_Band").reindex(A.DISCOUNT_BAND_LABELS)
        )
        fig.add_scatter(
            x=A.DISCOUNT_BAND_LABELS,
            y=series["Margin_Pct"],
            name=name,
            mode="lines+markers",
        ...
        fig.add_annotation(
            x=A.DISCOUNT_BAND_LABELS[-1],
            y=series["Margin_Pct"].iloc[-1],
            text=f"  {name}: {series['Margin_Pct'].iloc[-1]:.0f}%",
            showarrow=False,
            xanchor="left",
    ...
    fig.update_layout(
        title=(
            "One discount policy, two different businesses: Electronics stops earning above 10% off"
        ),
    ...
    _note(
        fig,
        "Bands are order-level discount percentage. Read as tolerance, not as a causal "
...
def fig_driver_ranking(df) -> go.Figure:
    ...
    fig = go.Figure(
        go.Bar(
            ...
            hovertemplate=(
                "<b>%{x:.1f} pts</b> spread<br>%{y}<br>"
                "₹%{customdata[0]:.1f}M profit gap<extra></extra>"
            ),
        )
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title="Revenue-weighted spread in category margin (percentage points)",
    ...
    _note(
        fig,
        "Spread = sqrt(Σ wᵍ (marginᵍ − company margin)²), weighted by each group's revenue "
...
def fig_misleading_pair(df) -> go.Figure:
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            'Exhibit A: "Premium is our worst segment"',
            "Exhibit B: the same orders, split by what was in them",
        ),
    )
    fig.add_bar(
        x=segments,
        y=confound["Margin_Pooled"],
        name="Pooled margin",
        marker=dict(color=INK["muted"], cornerradius=4),
        text=[f"{v:.1f}%" for v in confound["Margin_Pooled"]],
        textposition="outside",
        textfont=dict(color=INK["secondary"]),
    ...
    fig.add_bar(
        x=segments,
        y=confound["Margin_Ex_Electronics"],
        name="Everything except Electronics",
    ...
    fig.add_bar(
        x=segments,
        y=confound["Margin_In_Electronics"],
    ...
    fig.update_yaxes(
        ...
        range=[0, confound["Margin_Pooled"].max() * 1.35],
        row=1,
        col=1,
    )
    ...
    _note(
        fig,
        f"Premium's pooled margin is low because {premium_share:.0f}% of its revenue is "
...
def fig_on_time_monthly(df) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_scatter(
        x=monthly["Order_YearMonth"],
        y=monthly["On_Time_Pct"],
        mode="lines+markers",
        name="On-time rate by month",
    ...
    for _, row in monthly[monthly["Is_Peak"]].iterrows():
        fig.add_vrect(
            x0=row["Order_YearMonth"].isoformat(),
    ...
    fig.add_vline(
        x=A.LOGISTICS_START.isoformat(),
        line_dash="dash",
        line_width=1,
        line_color=INK["muted"],
        annotation_text="new logistics contract",
    ...
    _note(
        fig,
        "All orders, including cancellations. Shaded bands are October–November. "
...
def fig_channel_economics(df) -> go.Figure:
    ...
    for name, row in channels.iterrows():
        fig.add_scatter(
            x=[row["Margin_Before"], row["Margin_After"]],
            y=[name, name],
            mode="lines",
            line=dict(color=INK["axis"], width=2),
            showlegend=False,
            hoverinfo="skip",
        )
    fig.add_scatter(
        x=channels["Margin_Before"],
        y=list(channels.index),
        mode="markers",
    ...
    _note(
        fig,
        "Marketing cost as a share of the revenue it earned: "
        + ", ".join(
            f"{name} {row['Marketing_Pct_After']:.1f}%" for name, row in channels.iterrows()
        )
        + ".",
    )
    return finalize(fig, HEIGHT)
